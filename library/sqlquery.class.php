...
class SQLQuery {
	...
	function where($field, $value,$must=true,$custom_field = false,$condition = '=') {
		if ($must) $must = "AND"; else $must = "OR";
		 
		if ($custom_field)
			$this->_extraConditions .=  $field.' '. $condition .' \''.mysql_real_escape_string($value).'\'  '.$must.' ' ;
		else
			$this->_extraConditions .= '`'.$this->_model.'`.`'.$field.'` '. $condition .' \''.mysql_real_escape_string($value).'\'  '.$must.' ' ;
	}

	function like($field, $value,$must=true,$custom_field = false) {
		if ($must) $must = "AND"; else $must = "OR";
		if ($custom_field)
			$this->_extraConditions .=   $field.'` LIKE \'%'.mysql_real_escape_string($value).'%\' '.$must.' ';
		else
			$this->_extraConditions .= '`'.$this->_model.'`.`'.$field.'` LIKE \'%'.mysql_real_escape_string($value).'%\' '.$must.' ';
	}
	...
	function NumOfRecordAfterFilter() {
		if ($this->_query) {
			$this->_result = mysql_query($this->_query, $this->_dbHandle);
			 
				$count = mysql_numrows($this->_result);
