...
if (isset($_SESSION['IS_ADMIN']) && $_SESSION['IS_ADMIN']=='1'){
	...
	$tags = $news['News']['tags'];
	$front = $news['News']['front'];
	$main_msg= $news['News']['content'];
	
	$user_created = $news['News']['user_created'];
	$date_created = date("M d Y",strtotime($news['News']['date_created']));
	...
	<form name='adminform' method="POST" action='<?php echo BASE_PATH."/admin/news/edit/".$id ?>' onsubmit='return validate_form(this);'>
	...
	<input type=hidden size=133 name=user_created value='<?php echo $user_created?>'>
		...
		<p>
			...
			<input type=button value=Back class=button1 onclick="GoToURL('<?php echo BASE_PATH?>/admin/news')">
		</p>
	</form>
	<?php
...
<?php }else{
	?>
	<div id='errormsg'><?php echo $msg?></div>
	<form name='adminform' method="POST" action='<?php echo BASE_PATH."/admin/news/add"?>' onsubmit='return validate_form(this);'>
	<input type=button value=Back class=button1 onclick="GoToURL('<?php echo BASE_PATH ?>/admin/news')" ></input>
	</form>
	<?php 
}